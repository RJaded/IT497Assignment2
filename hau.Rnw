...
Government Spending, in simple words, can be termend as the inclusion of government consumpion, investments and transfer payments. Every country prepares an annual budget which consists funds for the various departments who would look for the well-being of the nation. This budget depends on the revenue growth of the country as well as its foreign relations. 
...
Lastly, \textbf{authcode} is optional, which authenticates the user to the Quandl website. 
...
SpendingData
@
In the above dataframe, the first column \texttt{Date}, consists of the range of dates as specified by the Quandl API. The second column \texttt{USA}, consists the government spending by USA for the corresponding date. The third column \texttt{Canada}, consists the government spending by Canada for the corresponding date. The final column \texttt{Mexico}, consists the government spending by Mexico for the corresponding date.
...
The graph above shows the Governemnt Spending in North America for USA, Canada and Mexico by change. In this graph, we notice that there was a constant growth in the governemnt spending in the North American countires. However, during 2007-10, it is seen that, there has been a significant rise as well as significant decresae in the government spedning of USA.
...
\\In Canada, during 1995-96, the Canadian governemnt decided to reduce the economic budget for the country. This was achieved with cutting the nominal spending and public sector employment of the Canada. Hence, we negative change in the year 1995 in the Government Spending (North America) - By Change graph.
